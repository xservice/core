...
var Monitor = require('@xservice/server');
var Monitor__default = _interopDefault(Monitor);
var Router = _interopDefault(require('@xservice/radix'));
...
class TargetMetadata {
    ...
    static bind(target) {
        let meta;
        if (!Reflect.hasMetadata(exports.NAMESPACE.TARGET, target)) {
            meta = new TargetMetadata();
            Reflect.defineMetadata(exports.NAMESPACE.TARGET, meta, target);
        }
        else {
            meta = Reflect.getMetadata(exports.NAMESPACE.TARGET, target);
        }
        return meta;
    }
}
class MethodMetadata {
    ...
    static bind(target) {
        let meta;
        if (!Reflect.hasMetadata(exports.NAMESPACE.METHOD, target)) {
            meta = new MethodMetadata();
            Reflect.defineMetadata(exports.NAMESPACE.METHOD, meta, target);
        }
        else {
            meta = Reflect.getMetadata(exports.NAMESPACE.METHOD, target);
        }
        return meta;
    }
}
class ParameterMetadata {
    ...
    static bind(target) {
        let meta;
        if (!Reflect.hasMetadata(exports.NAMESPACE.PARAMETER, target)) {
            meta = new ParameterMetadata();
            Reflect.defineMetadata(exports.NAMESPACE.PARAMETER, meta, target);
        }
        else {
            meta = Reflect.getMetadata(exports.NAMESPACE.PARAMETER, target);
        }
        return meta;
    }
}
...
class Service extends EventEmitter {
        ...
        this.router = new Router({
            ...
            defaultRoute: ctx => {
                if (this.listenerCount('404') > 0)
                    ...
                    throw new Monitor.VPCExpection('cannot find the router.path of ' + ctx.path, 404);
            },
        });
    }
    bind(target) {
        ...
        for (let i = 0; i < properties.length; i++) {
            ...
            if (property === 'constructor')
                continue;
            const methodMeta = Reflect.getMetadata(exports.NAMESPACE.METHOD, that);
            if (methodMeta) {
                const methodPathes = [];
                ...
                if (methodMeta.has('router.method.get'))
                    methodPathes.push({ method: 'router', path: methodMeta.get('router.method.get') });
                if (methodMeta.has('router.method.post'))
                    methodPathes.push({ method: 'router', path: methodMeta.get('router.method.post') });
                if (methodMeta.has('router.method.put'))
                    methodPathes.push({ method: 'router', path: methodMeta.get('router.method.put') });
                if (methodMeta.has('router.method.delete'))
                    methodPathes.push({ method: 'router', path: methodMeta.get('router.method.delete') });
                ...
                if (methodPathes.length === 1) {
                    ...
                    tmpMiddleware.push(async (ctx, next) => {
                        const parameterMeta = Reflect.getMetadata(exports.NAMESPACE.PARAMETER, that);
                        if (parameterMeta) {
    ...
    listen(mapState) {
        const createServer = Monitor__default({
...
exports.setDynamicMethodMetaData = setDynamicMethodMetaData;
exports.setDynamicParameterMetaData = setDynamicParameterMetaData;
exports.setDynamicTargetMetaData = setDynamicTargetMetaData;
exports.setStaticMethodMetaData = setStaticMethodMetaData;
exports.setStaticParameterMetaData = setStaticParameterMetaData;
exports.setStaticTargetMetaData = setStaticTargetMetaData;
exports.vpc = vpc;
